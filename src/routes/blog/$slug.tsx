    ...
    return {
      meta: [
        ...
        ...(post.frontmatter.author
          ? [{ property: 'article:author', content: post.frontmatter.author }]
          : []),
    ...
    <div className="relative min-h-screen overflow-hidden">
      ...
      <article className="mx-auto max-w-4xl px-6 py-16 sm:py-24">
            ...
            {post.frontmatter.tags && post.frontmatter.tags.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {post.frontmatter.tags.map((tag) => (
                  <Link key={tag} to="/blog" search={{ filter: tag.toLowerCase() }}>
                    <Badge
                      variant="secondary"
        ...
        <ArticleJsonLdComponent
          ...
          timeRequired={
            post.readingTime ? `PT${extractReadingMinutes(post.readingTime)}M` : undefined
          }
        />
        ...
        <BreadcrumbJsonLd
          items={[
            ...
            { position: 3, name: post.frontmatter.title, item: `${SITE_URL}/blog/${post.slug}` },
          ]}
        />

        <BlogProse html={post.content} className="blog-prose" />
